#!/usr/bin/env python3
"""
Multi-b-shell mean FA plot using **reconstructed** reference InMNI maps (SH-reconstructed DWI → FA),
same convention as debug_FA_modified.py with --ref-reconstructed.

For each subject in the **reference** caselist, loads:
  templatePath/reconstructed_<prefix>_InMNI_FA.nii.gz

Target before / after still use the usual:
  templatePath/<prefix>_InMNI_FA.nii.gz

Example:
  python plot_meanFA_multibshell_reconstr.py \\
...
def analyze_stat_reconstructed_ref(caselist_file, template_path, mni_tmp):
    """Mean FA on IIT skeleton; reference uses reconstructed_<prefix>_InMNI_FA.nii.gz."""
    ...
    for img_path, _mask in cases:
        ...
        fa_img = pjoin(template_path, 'reconstructed_' + prefix + '_InMNI_FA.nii.gz')
...
def analyze_stat_original_inmni(caselist_file, template_path, mni_tmp):
    """Mean FA on IIT skeleton; <prefix>_InMNI_FA.nii.gz (target before / harmonized)."""
    skel_path = pjoin(dirname(mni_tmp), 'IITmean_FA_skeleton.nii.gz')
    skel = load(skel_path)
    skel_mask = (skel.get_fdata() > 0) * 1.0
...
def main():
    parser = argparse.ArgumentParser(
        description='Multi-b-shell meanFA: reconstructed reference InMNI + original target before/after.',
    )
    ...
    parser.add_argument(
        '--ref-pattern',
        required=True,
        help='Reference caselist with {bshell}, e.g. harmonization_input_ref_b{bshell}.csv.modified',
    )
    ...
    parser.add_argument(
        '--out',
        default=None,
        help='Output PNG (default: templatePath/meanFA_multib_reconstr_ref.png)',
    )
    ...
    out_path = abspath(args.out) if args.out else pjoin(template_path, 'meanFA_multib_reconstr_ref.png')
    ...
    for b in bshells:
        ...
        if missing:
            print(f'WARNING b={b}: missing caselist(s), skipping:')
            for m in missing:
                print(f'    {m}')
            continue

        print(f'=== b = {b} (ref = reconstructed_*_InMNI_FA) ===')
        ref_m = analyze_stat_reconstructed_ref(ref_csv, template_path, mni_tmp)
        tar_m = analyze_stat_original_inmni(tar_csv, template_path, mni_tmp)
        harm_m = analyze_stat_original_inmni(harm_csv, template_path, mni_tmp)

        if not ref_m:
            print(f'  WARNING b={b}: no reconstructed reference InMNI FA loaded, skipping')
            continue

        print(
            f'  Reference (recon): mean={np.mean(ref_m):.4f} N={len(ref_m)} | '
    ...
    if not rows:
        print('ERROR: No valid b-shell. Ensure reconstructed_<prefix>_InMNI_FA.nii.gz exist in templatePath.')
        sys.exit(1)
    ...
    ax_sum = axes[-1]
    ax_sum.plot(bs_used, means_ref, 'o-', color='#1f77b4', label=f'Reference ({args.refSite}, recon)', linewidth=2)
    ...
    ax_sum.set_title('Group mean FA vs b-shell', fontsize=11)
    ...
    fig.suptitle(
        'Reconstructed reference (SH) vs target before / harmonized after — all b-shells',
        fontsize=13,
        y=1.002,
    )

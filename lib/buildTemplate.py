...
def dti_stat(siteName, imgs, masks, templatePath, templateHdr):
    ...
    for dm in diffusionMeasures:
        ...
        save_nifti(pjoin(templatePath, f'Std_{siteName}_{dm}_b{bshell_b}.nii.gz'),
...
def rish_stat(siteName, imgs, templatePath, templateHdr):
    ...
    for i in range(0, N_shm+1, 2):
        ...
        templateAffine= templateHdr.get_best_affine()
        save_nifti(pjoin(templatePath, f'Mean_{siteName}_L{i}_b{bshell_b}.nii.gz'),
                            np.mean(imgData, axis= 0), templateAffine, templateHdr)

        save_nifti(pjoin(templatePath, f'Std_{siteName}_L{i}_b{bshell_b}.nii.gz'),
                            np.std(imgData, axis= 0), templateAffine, templateHdr)
...
def difference_calc(refSite, targetSite, refImgs, targetImgs,
    ...
    for dm in measures:
        ...
        if travelHeads:
            ...
            fileName= pjoin(templatePath, f'Mean_{refSite}_{dm}_b{bshell_b}.nii.gz')
            ref= load_nifti(fileName)[0]

            fileName= pjoin(templatePath, f'Mean_{targetSite}_{dm}_b{bshell_b}.nii.gz')
        ...
        save_nifti(pjoin(templatePath, f'Delta_{dm}_b{bshell_b}.nii.gz'),
                   np.mean(delta, axis= 0), templateAffine, templateHdr)

        save_nifti(pjoin(templatePath, f'PercentageDiff_{dm}_b{bshell_b}.nii.gz'),
                   np.mean(per_diff, axis= 0), templateAffine, templateHdr)

        save_nifti(pjoin(templatePath, f'PercentageDiff_{dm}smooth_b{bshell_b}.nii.gz'),
                   np.mean(per_diff_smooth, axis= 0), templateAffine, templateHdr)

        if 'L' in dm:
            save_nifti(pjoin(templatePath, f'Scale_{dm}_b{bshell_b}.nii.gz'),

...
def main():
    ...
    log_info('ttest among statistics')
    for key in keys:
        log_info(key)
        log_info('before', ttest_ind(df_ref[key],df_tar[key])[1])
        log_info('after', ttest_ind(df_tar[key],df_harm[key])[1])
        log_info('')

if __name__=='__main__':
    if len(sys.argv)==1 or sys.argv[1]=='-h' or sys.argv[1]=='--help':
        log_info('Usage: '
              'ttest.py ref_stat.csv tar_stat.csv harm_stat.csv')
        exit()
